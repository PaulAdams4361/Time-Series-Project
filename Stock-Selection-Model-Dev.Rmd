...
plotts.sample.wge(df$low) # the estimated model (above) matches to the actual model (here) on both sample ACFs and sample spectral density as well as on partial ACF (below):
pacf(est_mod)
pacf(df$low)
...
We take the signal, test the hypothesis of the presence of deterministic signal in the realization. Then, we separate the noise from the signal and estimate and ARMA/ARIMA model using differencing. We then take the model estimates and generate a signal pluse noise model forecast.
...
est.arma.wge(df$low, p=5, q=0) # the factor table for df$low provides one (1-B) represented as (1-0.9956B), a close-enough approximation. Therefore, we difference the data once. Preliminary evidence suggesting differencing is useful is based on the specified wandering and the damping sample autocorrelations.

dftrans <- artrans.wge(df$low, phi.tr=1)
aic5.wge(dftrans, type="aic") # aic = -2.001944
estimated_arma <- est.arma.wge(dftrans, p=5, q=1)
estimated_arma$avar # 0.1172604
mean(dftrans) # 0.07590909
# (1-B)(1-1.04489743B+0.24211505B^2+0.05167856B^3+0.11537566B^4-0.17553271B^5)*(X_t + 0.0759) = a_t, (sigma-hat_a)^2 = 0.93
# (1-2.04489743B + 1.28701248B^2 - 0.19043649B^3 + 0.0636971B^4 - 0.29090837B^5 + 0.17553271B^6)(X_t + 0.0759) = a_t, (sigma-hat_a)^2 = 0.93

######
###### Plotting residuals

# the residuals of the model do not appear correlated. The sample autocorrelation and partial autocorrelation plots indicate stationarity across all lags with all lags measured within the 5% limits
...
# We generated a model of 99 data points using the ARMA(5,1) identified by aic5 with a variance equal to that estimated from the differenced data, which is 0.1172604 (select to see origin highlighted above)
est_mod2 <- gen.aruma.wge(99, phi=estimated_arma$phi, theta=estimated_arma$theta, d=1, vara=estimated_arma$avar, sn=30)
plotts.sample.wge(est_mod2, arlimits=T) # estimated model
plotts.sample.wge(dftrans, arlimits=T) # original differenced data
...
nonseasonal.forecast <- fore.aruma.wge(dftrans, phi=estimated_arma$phi, theta=estimated_arma$theta, n.ahead=5, lastn=T)
ARIMA_ASE = mean((dftrans[(length(dftrans)-5+1):length(dftrans)] - nonseasonal.forecast$f)^2)
ARIMA_ASE # 0.3454855
