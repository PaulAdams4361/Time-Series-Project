...
library(pacman)
p_load(tswge, dplyr, kableExtra, sqldf, ggplot2)
```
...
oldFile <- read.csv('./NASDAQ_Daily_ACGL.csv', header=T, strip.white=F)
plotts.sample.wge(oldFile$low, arlimits=T)

par(mfrow=c(2,2))
plot(oldFile$low,type = "l")
lines(oldFile$low,col = "blue", type = "l")
pacf(oldFile$low)
acf(oldFile$low)
parzen.wge(oldFile$low, trunc=45)

newFile <- read.csv('../Time-Series-Stocks/NASDAQ_Daily_ACGL.csv', header=T, strip.white=F)
plotts.sample.wge(newFile$low, arlimits=T)

par(mfrow=c(2,2))
plot(newFile$low,type = "l")
lines(newFile$low,col = "blue", type = "l")
pacf(newFile$low)
acf(newFile$low)
parzen.wge(newFile$low, trunc=45)
