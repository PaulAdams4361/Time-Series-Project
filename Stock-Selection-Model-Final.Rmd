...
```{r Multiple Linear Regression Time Series without Time, echo=T, warning=F}
df <- stock_data()
...
```{r Multiple Linear Regression with Trend, Time and Volume Interaction, echo=T, warning=F}
df <- stock_data()
...
```{r Multiple Linear Regression with Trend, Time and Volume Interaction, Time and OpClo Interaction, echo=T, warning=F}
df <- stock_data()
t=1:nrow(df) # df has 100 rows. Creating a time vector with this for trending
