...
a linear model on each stock and where the slope for price is positive with a slope greater than 0.04 and the price is within an affordable 
...
for( i in 1:5)
{
  ...
  lsfit <- VAR(y=cbind(df2$low[i:(i + (trainingSize - 1))], df2$volume[i:(i + (trainingSize - 1))], df2$HiLo[i:(i + (trainingSize - 1))], df2$OpClo[i:(i + (trainingSize - 1))]), p=2, type="const")
  preds <- predict(lsfit, n.ahead=5)
...
Our model with Trend and Trend and volume interaction, we included time as an additive predictor in addition to a multiplicative term between trend and share volume traded. This reduced the ASE, which by this metric, indicated this is the best VAR model. The model identifed is an AR(2) with an AIC score of -2.4780.
...
Finally, we modeled with an interaction between trend and volume and trend and the range between open and close prices (we also attempted for interaction between trend and the range between high and low prices, but this hurt the model, potentially indicating there is greater variation in these two prices than the outer two - open and close). However, the ASE resulting from the forecast was lower than the multiplicative model only including an interaction between trend and trade volume. The model identified is an AR(2) with an AIC score of -2.4645.
