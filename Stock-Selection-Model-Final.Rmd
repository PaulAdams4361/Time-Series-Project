...
for (i in 0: (num_batches-1))
{
  ...
  preds <- predict(fit, newxreg=cbind(t[(nrow(df)-4):nrow(df)], df$volume[(nrow(df)-4):nrow(df)], df$HiLo[(nrow(df)-4):nrow(df)], 
                                      df$OpClo[(nrow(df)-4):nrow(df)], t[(nrow(df)-4):nrow(df)]*df$volume[(nrow(df)-4):nrow(df)],
                                      t[(nrow(df)-4):nrow(df)]*df$OpClo[(nrow(df)-4):nrow(df)]))
  
  ASEs[i+1] = mean((df$low[(nrow(df)-4):nrow(df)] - preds$pred)^2)
}
ASEs
mean(ASEs)
...
signoise = as.numeric(preds$pred)
