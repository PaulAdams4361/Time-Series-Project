...
plot(seq(1,nrow(df),1), df$low, type="b")
points(seq((nrow(df) - 4), nrow(df), 1), preds$pred, type="b", pch=15)

```

```{r Multiple Linear Regression Time Series Including Trending with Time, echo=T, warning=F}
df <- stock_data()
...
ASE <- mean((BSales$sales[96:100] - preds$pred)^2)
ASE
